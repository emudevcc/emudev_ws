...
2. **Strength** — `sanity/schemas/strength-type.ts`:
   ...
   export const strengthType = defineType({
     ...
     fields: [
       defineField({ name: 'name', type: 'string', validation: (r) => r.required() }),
       defineField({ name: 'rank', type: 'number', validation: (r) => r.required().integer().min(1).max(5) }),
       defineField({ name: 'domain', type: 'string', options: { list: DOMAINS } }),
       localizedRichText('description', 'Description'),
     ],
     preview: { select: { title: 'name', rank: 'rank', subtitle: 'domain' },
...
3. **SocialPost** — `sanity/schemas/social-post-type.ts`:
   ...
   export const socialPostType = defineType({
     ...
     type: 'document',
     fields: [
       defineField({ name: 'platform', type: 'string', options: { list: PLATFORMS }, validation: (r) => r.required() }),
     ...
     orderings: [
       { title: 'Posted, newest first', name: 'postedAtDesc', by: [{ field: 'postedAt', direction: 'desc' }] },
     ],
   })
   ```
...
- [ ] Create `language-type.ts` (4 fields, proficiency enum)
- [ ] Create `strength-type.ts` (4 fields, custom rank preview, ordering)
- [ ] Create `social-post-type.ts` (8 fields, conditional `subreddit`, stats object)
- [ ] Create `testimonial-type.ts` (6 fields, experience ref)
- [ ] `npm run typecheck` passes

## Success Criteria

- [ ] Strength.rank rejects values outside 1-5
- [ ] SocialPost.subreddit hidden in Studio when platform != 'reddit'
- [ ] Testimonial.relatedExperience picker shows Experience docs (post-Phase 7)
- [ ] All preview cards render correctly in Studio document list
