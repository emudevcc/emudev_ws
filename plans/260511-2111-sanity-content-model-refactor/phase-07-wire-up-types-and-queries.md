...
1. **Register types** — `sanity/schema.ts`:
   ...
   export const schema = {
     types: [
       ...
       // New (Phase 2-5)
       aboutType, skillType, experienceType,
       certificationType, educationType,
       languageType, strengthType, socialPostType, testimonialType,
     ],
   }
   ```
     ...
     S.list()
       ...
       .items([
         ...
         S.listItem()
           ...
           .child(
             S.list().title('Skills & Credentials').items([
         ...
         S.listItem()
           .title('About Extras')
           .child(
             S.list().title('About Extras').items([
...
- [ ] Register all 9 new types in `schema.ts`
- [ ] Create `sanity/structure.ts` with 5 groups + 2 singletons
- [ ] Update `sanity.config.ts` to use custom structure
- [ ] Bump `cacheVersion` to `localized-v3`
- [ ] Expand `getSiteSettings` GROQ query
- [ ] Add 9 new query functions (experiences, skills, about, certifications, educations, languages, strengths, socialPosts, testimonials)
- [ ] Run `npm run sanity:types`
- [ ] Run `npm run typecheck`
- [ ] Run `npm run lint`
- [ ] Run `npm run build`
- [ ] Studio smoke test
- [ ] Front-end smoke test (existing pages still load)

## Success Criteria

- [ ] All 14 types listed in Studio under the 5 logical groups
- [ ] `siteSettings` and `about` enforced as singletons (clicking opens the same doc each time)
- [ ] `types/sanity.types.ts` regenerated; `git diff` shows new interfaces for all added types
- [ ] `npm run build` succeeds with zero TypeScript errors
- [ ] `lib/sanity-queries.ts` exports new functions matching the established pattern
- [ ] Cache tags wired (Phase 6 doc updates revalidate via existing webhook + new tag names)
- [ ] Existing projects, posts, siteSettings still resolve via legacy + new fields
